...
Nodex یک سرویس سبک و داکر‌شده است که به‌عنوان «نود» برای پنل مرکزی 3X-UI عمل می‌کند. این سرویس:
...
- داده‌ها و DB: `/var/lib/dds-nodex/data`  
...
**نکته**: مسیر پنل معمولاً شبیه `http://IP:PORT` یا `http://IP:PORT/panel` است. `WEBPATH` را مطابق پنل خود بگذارید.

### 2) متغیرهای محیطی (`.env`)

<div dir="rtl">

(اختیاری، برای تنظیمات پیشرفته)
...
### 3) docker-compose (خلاصه)

<div dir="rtl">

Nodex با این Compose اجرا می‌شود (خلاصهٔ کانفیگ):
...
### برای نصب نسخه های مختلف از یک اسکریپت استفاده کنید
...
1. **Login به پنل مرکزی و نودها** (Sessionهای پایدار با TTL)
...
PR و Issue با کمال میل پذیرفته می‌شود. لطفاً برای پیشنهاد فیچر/باگ، لاگ و جزئیات محیط را ضمیمه کنید.
...
- **Cryptocurrency / TON / USDT**: `YOUR_WALLET_ADDRESS_HERE`
- **درگاه داخلی**: [حمایت از Nodex](https://your-domain.com/donate)
- **BuyMeACoffee**: [buymeacoffee.com/yourname](https://buymeacoffee.com/yourname)

**نکته**: لینک‌ها را با اطلاعات خودتان جایگزین کنید. اگر دامنهٔ شخصی دارید، یک صفحهٔ سادهٔ `.me` برای دونیت بسازید و همینجا لینک دهید.
